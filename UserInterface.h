...
static const wchar_t *TEXT_NEWGAME = L"Новая игра";
static const short TEXT_NEWGAME_LENGTH = 10;

static const wchar_t *TEXT_RESET = L"Сброс";
static const short TEXT_RESET_LENGTH = 5;

static const wchar_t *TEXT_CANCELMOVE = L"Отменить";
static const short TEXT_CANCELMOVE_LENGTH = 8;

static const wchar_t *TEXT_HELP = L"Заполните игровое поле плитками, щелкая мышью.\nСледующая плитка находится здесь:";
static const short TEXT_HELP_LENGTH = 81;

static const wchar_t *TEXT_HELP_2 = L"Теперь перетащите плитки или щелкните на них мышью.";
static const short TEXT_HELP_2_LENGTH = 51;
...
inline void UserInterface<T>::StatUpdate(char utype, unsigned long long data) {
	...
	switch (utype) {
		...
		default:
			tmp = (gamefeild_pos << 2) + 142;
			TextOutW(hDc, buttons_pos, tmp, L"Могут быть отменены: 0", 35);
			TextOutW(hDc, buttons_pos + 17, tmp + 18, L"Выполнено движений: 0", 23);
			break;
	}
}
