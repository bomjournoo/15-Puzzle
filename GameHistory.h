...
inline void GameHistory<T>::Event_tileMoved(unsigned char direction) {
	if (++byte_data_pos == 4) {
		...
		#pragma warning (pop)
			if (!ldata_pos) {
				if (undo_array_size)
					(receiver->*msg)(L"Вы сделали слишком много шагов.\nЯ не могу вспомнить так много...\nYou are a machine as much as I am ...", L"Говорят, что программы не умеют удивляться, но я смог");
				else 
					(receiver->*msg)(L"Вы сделали слишком много шагов.\nЧтобы продолжить, добавьте немного оперативной памяти на этот компьютер, но пока все мои учетные данные ...", L"Говорят, что программы не умеют удивляться, но я смог");
				this->~GameHistory();
			}
