	...
	if (gameReady) {
		...
		if ((t1 == 0 && t2 == 1) || (t1 == 1 && t2 == 0)) {
			...
			*citem = 0;
			...
			if (CheckWin()) {
				std::wstring message = L"Вы выиграли благодаря этому ходу!\nПоздравляем!\nДвижения были сделаны: ";
				message += std::to_wstring(counterMove);
				ui->ShowMessage(message.c_str(), L"Вы победили!", 1);
			}
		}
	} else {
		if (arrange_stage) {
			if (!*citem) {
				*citem = ++arrange_stage;
				...
				if (arrange_stage != 15) {
					...
					if (CheckWin()) {
						ui->ShowMessage(L"Вы - мошенник! Вы положили свои плитки так, что сразу же выиграли.", L"Скам детектор", 1);
					}
				}
			} else {
				ui->ShowMessage(L"Эта позиция уже занята!", L"Неправильное размещение", 0);
			}
		} else {
